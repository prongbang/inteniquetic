...
function category(name: string, description: string, urls: string[]): Category {
  return { name, description, links: urls.map((url) => ({ url })) }
}

export function getCategories(): Category[] {
  return [
    category('Rust', 'Rust crates and CLI/Lib tools.', [
    ...
    category('GO', 'Libraries and utilities for Go, Fiber, Echo, and security.', [
    ...
    category('Flutter', 'Plugins and widgets focused on security, performance, and UX.', [
    ...
    category('JavaScript', 'Crypto libs (JS/TS) and small utilities.', [
      'https://github.com/prongbang/lazyxchacha-js',
      'https://github.com/prongbang/lazyaesgcm-js',
    ]),
    category('Swift', 'Secure storage / biometric / screen protection kits for iOS.', [
    ...
    category('Kotlin', 'Android components, MVI, secure storage/biometric, and UI utilities.', [
    ...
    category('Program', 'Standalone apps/scripts (e.g., AI and media).', [
